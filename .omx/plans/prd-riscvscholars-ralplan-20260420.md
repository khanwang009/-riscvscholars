...
- Narrowed the MVP to the existing Vite SPA and kept the current route skeleton instead of adding new top-level routes.
- Split the content schema into phase-1 required fields versus deferred editorial fields.
- Replaced the vague submission-adapter wording with one explicit MVP transport contract: `VITE_INTAKE_WEBHOOK_URL`.
- Added shared-source-of-truth requirements for `Research Lines` taxonomy and directory filters.
- Added verification coverage for empty states and non-open profiles.
